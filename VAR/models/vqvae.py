...
class VQVAE(nn.Module):
    def __init__(
        ...
        v_patch_nums=(1,2,4,6,8,10,13,16), # number of patches for each scale, h_{1 to K} = w_{1 to K} = v_patch_nums[k]
        test_mode=True,
        ...
        self.using_znorm=using_znorm# whether to normalize when computing the nearest neighbors
        self.quant_conv_ks=quant_conv_ks# quant conv kernel size
        self.quant_resi=quant_resi# 0.5 means \phi(x) = 0.5conv(x) + (1-0.5)x
        self.default_qresi_counts=default_qresi_counts# use 4 \phi layers for K scales: partially-shared \phi
        self.test_mode=test_mode# if is 0: automatically set to len(v_patch_nums)
        self.v_patch_nums=v_patch_nums# number of patches for each scale, h_{1 to K} = w_{1 to K} = v_patch_nums[k]
        self.ch=ch
        self.image_gan_weight=0.3
        ...
        ddconfig = dict(
            dropout=self.dropout, ch=ch, z_channels=z_channels,
        ...
        self.quantize: VectorQuantizer2 = VectorQuantizer2(
            vocab_size=vocab_size, Cvae=self.Cvae, using_znorm=self.using_znorm, beta=self.beta,
            default_qresi_counts=self.default_qresi_counts, v_patch_nums=self.v_patch_nums, quant_resi=self.quant_resi, share_quant_resi=share_quant_resi,
        )
        self.quant_conv = torch.nn.Conv2d(self.Cvae, self.Cvae, self.quant_conv_ks, stride=1, padding=self.quant_conv_ks//2)
        self.post_quant_conv = torch.nn.Conv2d(self.Cvae, self.Cvae, self.quant_conv_ks, stride=1, padding=self.quant_conv_ks//2)
