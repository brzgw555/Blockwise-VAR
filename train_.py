...
def build_everything(args: arg_util.Args):
    ...
    from VAR.models import VAR, VQVAE, build_vae_var
    from trainer_ import VARTrainer
    ...
    vae_ckpt = './block_wise_results_8scale_ema_retrain_add/checkpoints/model_step_599999.ckpt'
    if dist.is_local_master():
        if not os.path.exists(vae_ckpt):
            os.system(f'wget https://huggingface.co/FoundationVision/var/resolve/main/{vae_ckpt}')
    dist.barrier()
    state_dict = torch.load(vae_ckpt, map_location='cpu',weights_only=True)
    vae_weights = state_dict.get("vae",None)
    vae_local.load_state_dict(vae_weights, strict=True)
...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        is_saving =(ep + 1) % 10 == 0 or (ep + 1) == args.ep
        if is_val_and_also_saving:
            ...
            print(f' [*] [ep{ep}]  (val {tot})  Lm: {L_mean:.4f}, Lt: {L_tail:.4f}, Acc m&t: {acc_mean:.2f} {acc_tail:.2f},  Val cost: {cost:.2f}s , fid: {fid_score}')
            if dist.is_master():
                trainer.run_log.log({f"AR_iter_loss/fid":fid_score},step=ep+1)

            if dist.is_local_master() and is_saving:
